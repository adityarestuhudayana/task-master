...
import { updateBoardSchema, type UpdateBoardValues } from "@/lib/validators"
...
export function BoardSettingsModal() {
    ...
    const { boardSettingsOpen, boardSettingsId, closeBoardSettings } = useUIStore()

    const { data: board } = useBoard(boardSettingsId!)
    ...
    const onSubmit = async (data: UpdateBoardValues) => {
        try {
            await updateBoard.mutateAsync({ id: boardSettingsId!, ...data })
            onClose()
        } catch {
    ...
    const onDelete = async () => {
        ...
        try {
            await deleteBoard.mutateAsync(boardSettingsId!)
            onClose()
        } catch {
            // Error handled by mutation
        }
    ...
    if (!boardSettingsOpen) return null
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500">
                                Update board details and preferences
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-5 space-y-6">
                    ...
                    <div className="flex items-center justify-between pt-2">
                        <button
                            ...
                            className="flex items-center gap-2 px-3 py-2 rounded-lg text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 text-sm font-medium transition-colors disabled:opacity-50"
                        >
                            <Trash2 size={16} />
                            Delete Board
                        </button>
